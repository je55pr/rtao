...
- ordinary race launches now snapshot the same complete native player equipment block used by free-roam, including direct race entry and Q's Factory handoff; scalar categories 1..6 feed the recovered vehicle core while retained tyre/wheel selectors also keep the player race model aligned with the saved loadout; the shared special-ability contract derives Wing Set, Propeller, Jet Turbine and Water Ski flags for their proven race consumers while Flight Wing and unresolved presentation selectors fail closed;
...
Ordinary free-roam longitudinal/steering motion runs through an executable-backed `NativeDrivingMotion` boundary at PAL's 50 Hz update rate. Standard-FLD driving now advances the retained seven-probe support/contact/orientation recurrence once per PAL tick against the original FLD collision packet/plane records; raw native surface words, support history, local velocity delta, gravity response, matrix/inverse state and auxiliary-contact state feed `NativeDrivingMotion` directly, with no level-support compatibility inputs on that path. The same three retained support channels build PAL's local chassis/body matrix each tick, including Big Tyre's recovered `+0.85` body lift, while the contact matrix remains the terrain/root frame; the rendered Q62 composes both and reflects the body matrix only at the Three.js boundary instead of using browser-authored ride smoothing. Native Tyre/Engine/Chassis/Transmission/Steering/Brake selectors 1..6 still feed the recovered equipment records, the player path retains the symmetric-drift policy, and the existing retail PAL oracle path remains unchanged. Fixed position and contact stay in unreflected PAL coordinates through the recurrence. Big Tyre's recovered `0x0400` threshold, deep `0x100651` grip-slot replacement, Propeller `0x0040` special-contact thrust and Water Ski `0x0100` steering authority are carried into the native contact path. Standard-FLD terrain-edge/contact flags and authored obstacle masks now continue through the recovered outdoor scene-kind `-1` rollback/push response, with response velocity and yaw retained into the next tick; support loss/reacquisition, the 65-tick unsupported recovery pulse, landing impulses and auxiliary-height water transitions remain native support/history state rather than browser halt, ballistic, retreat or road-snap policy. Compiled-only fixtures and special-outdoor scenes deliberately retain the previous browser footprint/attitude bridge because their native dispatch is not recovered. The slot-`11` 26-group obstacle extension has an explicit runtime-enable seam, but the PAL enable bitset is not fabricated; free-roam scene suppression/reset producers and the vertical-impulse producer remain fail-closed, and the browser's north/south torus convenience is not fed into native contact. Reverse command production remains host policy. Category 7 fitting now preserves all 15 recovered WHEEL.BIN selectors through exact selector-to-section geometry, and fitted category-11 sponsor signs use the recovered driven-distance/Cake progression loop. Ordinary races expose the recovered Jet Turbine command `0x08` through the existing Boost action while the native frame retains its equipment, scene, fuel and sound gates. Unrecovered category-8 light presentation, Wing/Special/Option accessory geometry, sticker rendering, horn audio, meter HUD presentation and Police Light behavior remain evidence-gated on issue #30. Flight Wing's fitted/active transition and downstream contact/orientation consumers are recovered in the lower race runtime, but production launch remains fail-closed until the exact PAL `0x0021E208` velocity-derived scalar source is recovered. The implementation boundary and remaining gaps are recorded in `docs/archaeology/PAL_NATIVE_FREE_ROAM_CONTACT_HOST_CONTRACT_2026-09-20.md`, `docs/archaeology/PAL_NATIVE_FREE_ROAM_COLLISION_AIRBORNE_2026-09-20.md`, and `docs/archaeology/PAL_NATIVE_SPECIAL_ABILITY_RUNTIME_CONTRACT_2026-09-20.md`.
...
1. Continue issue #30 archaeology for Flight Wing helper `0x0021E208` and native presentation, plus the unresolved light, accessory, sticker, horn and meter presentation consumers; ready category 7–14 selector/setup/runtime paths remain separate from those evidence gates.
...
- Flight Wing's exact `0x0021E208` scalar producer and native presentation, plus unresolved category-8/12/13/14 and accessory presentation consumers; fitted/active `0x0004/0x0008` transition/downstream consumers and race-frame `0x1000/0x2000` branches are recovered;
