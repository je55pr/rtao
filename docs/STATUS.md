...
Ordinary free-roam and races also share the recovered native chase-camera recurrence and preset data. The browser output projection remains a host rendering seam pending a measured end-to-end PAL camera trace; free-roam additionally applies the separately named `applyBrowserChaseObstructionSafety` adapter, which is not claimed as the PAL obstruction loop. No speed-dependent FOV, camera pull-back, shake, synthetic collision response or other “feel” multiplier was added by the presentation pass.
