...
Users/developers supply their own PAL game inputs locally. Hosted GitHub Actions runs only tests/builds that do not require those copyrighted inputs.
...
- PR descriptions must state when PAL validation could not be run rather than implying hosted CI proves native parity.
