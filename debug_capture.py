...
async def main():
 async with async_playwright() as p:
  ...
  await page.evaluate("""async()=>{const files=Array.from(document.querySelector('#files').files); return await window.__rtaSandboxCapture.prepareOutdoorFromBrowserFiles(files,false,[223]);}"""); print('prepared',flush=True)
  t=time.time(); r=await page.evaluate("""async()=>await window.__rtaSandboxCapture.capturePreparedOutdoor('peach-night-ground')"""); print('captured',time.time()-t,len(r['pngBytes']),flush=True)
  Path('/mnt/data/rta_night_visibility_captures/debug.png').write_bytes(bytes(r['pngBytes'])); await browser.close()
asyncio.run(main())
