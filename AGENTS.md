...
- GitHub `dev` is the default branch, active integration branch for ongoing development, and source for the DEV player.
...
Trusted managers and agents may integrate validated work directly into `dev`. Task branches and PRs are optional tools for isolation, review, or overlapping work rather than mandatory merge ceremony. When a task branch is used, base it on `dev` and preserve the agent ownership prefix. Keep commits small and coherent enough to review and revert independently.

Promotion from `dev` to `main` is a deliberate release operation, not normal task integration. Require the complete browser gate, relevant PAL-backed validation, and a human DEV play/visual check, then fast-forward `main` to the exact validated `dev` commit. Do not squash or rebase release promotions: `main` should remain an ancestor of `dev` so later releases stay simple fast-forwards.
